...
test_num = 100
train_features, test_features = features[:-test_num], features[-test_num:]
train_targets, test_targets = targets[:-test_num], targets[-test_num:]
...
for ratio in np.arange(70, 100, 5):
    ...
    data_idxes.sort()
    with open(f"{index_dir}/{args.dataset_key}_{args.preset_key}_ours_ratio_{int(ratio)}_clusters_{args.num_clusters}_num_subsets_{int(args.num_subsets)}.txt", "w") as f:
...
# Strategy QA
# [0, 1, 2, 4, 6, 10, 13, 15, 16, 17, 19, 22, 23, 24, 25, 27, 38, 71]
# Commonsense QA
# [0, 4, 7, 16, 18, 29, 31, 32, 34, 44, 45, 46, 51]

indices = [0, 1, 2, 4, 6, 10, 13, 15, 16, 17, 19, 22, 23, 24, 25, 27, 38, 71]
data_idxes = []
for idx in indices:
    tmp_idxes = np.load(f"../gradients/{args.dataset_key}_{args.model_key}_{args.preset_key}_{args.project_dim}/clusters_{args.num_clusters}/cluster_{idx}.npy") 
    data_idxes.append(tmp_idxes)
...
data_idxes = np.concatenate(data_idxes)
data_idxes.sort()
with open(f"{index_dir}/{args.dataset_key}_{args.preset_key}_forward_selection.txt", "w") as f:
    f.write(" ".join([str(idx) for idx in data_idxes]))