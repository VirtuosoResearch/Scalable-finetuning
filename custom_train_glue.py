...
if __name__ == "__main__":
    ...
    for run in range(args.runs):
        ...
        if load_model_dir is not None and os.path.exists(load_model_dir + ".ckpt"):
            lm = GLUEModel.load_from_checkpoint(load_model_dir + ".ckpt", model=model, tokenizer=tokenizer, model_type=model_type,
                    lr=args.lr, weight_decay=args.weight_decay, max_length=args.max_length, use_wandb=args.use_wandb, answer_choices=answer_choices,
                    optimizer=args.optimizer, generate_output=args.generate_output)
