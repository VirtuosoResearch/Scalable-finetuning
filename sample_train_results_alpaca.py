        ...
        for line in f.readlines():
            ...
            os.system('''
                python custom_train_alpaca.py --model_key {} \
                    {}\
                    --lr {} --batch_size {} --max_length {} --epochs {} --precision {} --accumulate {}\
                    --devices {} --runs 1 --task_idxes {} \
                    --save_name {} --write_results
                '''.format(
                args.model_key,
                "--train_lora --lora_rank {} --lora_alpha {}".format(args.lora_rank, args.lora_alpha) if args.train_lora else "",
                args.lr,  args.batch_size, args.max_length, args.epochs, args.precision, args.accumulate,
                " ".join([str(idx) for idx in args.devices]),
                " ".join([str(idx) for idx in subset_idxes]),
                args.save_name # --freeze_head
            ))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subset_num", type=int, default=1000)
