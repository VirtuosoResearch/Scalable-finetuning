...
SUPPORTED_KEYS = ["zs", "zs_cot", "fs_cot", "ft", "ft_cot"]
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, dataset_key: str, preset_key: str, tokenizer, model_type: str, batch_size: int = 32,
                 inference_batch_size=None, num_workers: int = 8, append_eos=False, data_index_dir=None):
        """
        ...
        if data_index_dir: 
            self.data_index_dir = os.path.join("./data_indices", data_index_dir + ".txt")
            if not os.path.exists(self.data_index_dir):
                self.data_index_dir = None
        else:
            self.data_index_dir = None
    ...
    def setup(self, stage: str = None):
        ...
        if stage == "fit":
            ...
            if self.data_index_dir:
                with open(self.data_index_dir, "r") as f:
                    line = f.readline().strip()
                    line = line.split(" ")
                    line = [int(idx) for idx in line]
                self.train_dataset = dataset.select(line)
    ...
    def _compile_data(self, train=False):
        ...
        if self.preset_key in ["zs", "fs_cot", "ft"] or not train:
            ...
            samples = completion_dataset.select_samples(sample_indices, completion_indices, only_correct=False) # include all examples

        if samples is None:
            raise NotImplementedError(self.preset_key)

        # Format samples for model input/output.
        formatter = Formatter(self.model_type, self.prediction_template, dataset_key=self.dataset_key) if train \
            else Formatter(self.model_type, "ft_token", dataset_key=self.dataset_key)
        formatted = formatter.format_samples(samples, include_labels=True)
